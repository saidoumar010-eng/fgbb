  ...
  return (
    <Screen>
      ...
      <View style={{ padding: S.lg }}>
        <Button title={t('Nouveau match')} icon="add" onPress={() => router.push('/admin/match-form')} />
      </View>
      ...
      {matches.length === 0 ? (
        <Empty
          ...
          subtitle={loading ? undefined : t('Crée un match pour pouvoir saisir son score et ses statistiques.')}
        />
      ) : (
                ...
                <Row style={{ padding: 12, gap: 12 }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ color: C.dim, fontSize: 12, marginTop: 3 }}>
                      {m.competition?.name ?? t('Sans compétition')}
                    </Text>
                  </View>
                  {m.status === 'live' && <Pill label="LIVE" tone="red" dot />}
                  {m.status === 'finished' && <Pill label={t('Terminé')} tone="neutral" />}
                  {m.status === 'scheduled' && <Pill label={t('À venir')} tone="accent" />}
